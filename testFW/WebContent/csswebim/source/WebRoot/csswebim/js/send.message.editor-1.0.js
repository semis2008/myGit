/** * IM chat Send Message iframe editor * @author: hoojo * @email: hoojo_@126.com * @blog: http://blog.csdn.net/IBM_hoojo  * @createDate: 2012-5-24 * @version 1.0 **/
...
var sendMessageEditor = {
    ...
    execCmd: function(cmd, value, d){
        ...
        //doc对象的获取参照上面的代码        //调用execCommand方法执行命令      
    ...
    insertAtCursor: function(text, d, w){
        ...
        if (/msie/.test(agent)) {
            ...
            if (/gecko/.test(agent) || /opera/.test(agent)) {
                win.focus();
